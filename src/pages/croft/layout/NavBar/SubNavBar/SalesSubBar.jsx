...
const SalesSubBar = ({ years, setYears, period, setPeriod }) => {
  ...
  const handleToggleClick = (text) => {
    setYears(text);
    setPeriod(text);
    if (currentPath !== '/portfolio/croft/single-sales') navigate('/portfolio/croft/single-sales');
  };
  ...
  const handlePeriodClick = (text) => {
    ...
    if (text === '3' && new Date().getFullYear().toString().slice(-2) - 19 >= 2)
      navigate('/portfolio/croft/single-sales/years/3');
    if (text === '5' && new Date().getFullYear().toString().slice(-2) - 19 >= 4)
      navigate('/portfolio/croft/single-sales/years/5');
    if (
      text === '10' &&
      new Date().getFullYear().toString().slice(-2) - 19 >= 9
    )
      navigate('/portfolio/croft/single-sales/years/10');
  };
  return (
