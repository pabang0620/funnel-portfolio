...
const commonNavItems = [
  { to: '/funnel-edu/curriculums', icon: BookOpen, label: '커리큘럼' },
  { to: '/funnel-edu/resources', icon: FolderOpen, label: '교육자료실' },
]

const adminNavItems = [
  { to: '/funnel-edu/students', icon: Users, label: '수강생 관리' },
  { to: '/funnel-edu/admin-roles', icon: ShieldCheck, label: '권한 관리' },
]

const studentNavItems = [
  { to: '/funnel-edu/my-assignments', icon: UserRound, label: '내 과제' },
]
...
export default function AdminLayout() {
  ...
  const allItems = [
    ...(isAdmin && !viewAsStudent ? [{ to: '/funnel-edu/dashboard', icon: LayoutDashboard, label: '대시보드' }] : []),
    ...commonNavItems,
      ...
      <header className="sticky top-0 z-50 h-14 bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto h-full flex items-center px-8">
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <span className="font-bold text-gray-900 text-sm">FunnelEdu</span>
          </div>
