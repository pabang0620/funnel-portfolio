...
const menuItems = [
  {
    path: "/manceway/executive-report",
    label: "경영 리포트",
    icon: faChartLine,
  },
  { path: "/manceway/marketing-report", label: "마케팅 리포트", icon: faBullhorn },
  // { path: "/media-operations", label: "매체 운영 현황", icon: faEye },
  { path: "/manceway/data-entry-status", label: "데이터 입력 현황", icon: faDatabase, locked: true },
  // { path: "/customer-management", label: "고객 관리", icon: faUsers },
  {
    label: "CS 관리",
    path: "/manceway/cs-management/data",
    icon: faUsers,
    locked: true,
    subItems: [
      {
      ...
      {
        path: "/manceway/cs-management/shipping",
        label: "출고 관리",
        icon: faQrcode,
      },
    ],
  ...
  {
    label: "관리 리소스",
    path: "/manceway/management-resources/account-creation",
    icon: faUsersCog,
    locked: true,
    subItems: [
      {
      ...
      {
        path: "/manceway/management-resources/permission-settings",
        label: "등급별 권한 설정",
        icon: faUserShield,
      },
      {
        path: "/manceway/management-resources/product-management",
        label: "코드 및 제품 등록",
        icon: faQrcode,
      },
      {
        path: "/manceway/management-resources/marketplace-admedia",
        label: "판매처 및 매체 등록",
        icon: faPlus,
      },
      {
        path: "/manceway/management-resources/daily-quotes",
        label: "오늘의 명언",
        icon: faQuoteLeft,
      },
    ],
    ...
    <nav className={`sidebar ${isCollapsed ? "collapsed" : ""}`}>
      <div className="sidebar-content">
        <div className="sidebar-header">
          <h1 className="sidebar-title">{isCollapsed ? "Mw" : "Manceway"}</h1>
        </div>
