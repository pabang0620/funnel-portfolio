...
import { generateChannelColumns, generateChannelDataFromApi } from '../utils/tableColumnGenerators';
import { getChannelPerformance } from '../../api';

/**
 * 3번 테이블 (판매처별 실적현황) 데이터 관리 훅
 ...
 */
const useChannelTable = (customDateRange, selectedChannels, selectedProductId, salesChannelList, viewMode = 'channel', isExpanded = true) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await getChannelPerformance(params);
      ...
      console.error('판매처별 실적현황 데이터 조회 오류:', error);
      setApiData([]);
    } finally {
      setIsLoading(false);
    }
  }, [customDateRange, selectedProductId, viewMode, salesChannelList]);
  ...
  const columns = useMemo(
    () => generateChannelColumns(activeChannelInfo.names),
    [activeChannelInfo.names]
  );

  const data = useMemo(
    () => generateChannelDataFromApi(apiData, activeChannelInfo.names, viewMode, activeChannelInfo.ids),
    [apiData, activeChannelInfo.names, viewMode, activeChannelInfo.ids]
  );
