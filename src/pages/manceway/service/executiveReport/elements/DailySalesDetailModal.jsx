...
/**
 * 일별 판매 상세 모달 컴포넌트
 * 판매처별, 박스별 수량을 표시 (실제 데이터 연동, 데이터 있는 것만 표시)
 */
const DailySalesDetailModal = ({
  isOpen,
  onClose,
  ...
  const productNames = useMemo(() => {
    if (!selectedProducts || !productList.length) return [];
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (!selectedSalesChannels || selectedSalesChannels.length === 0) {
        return;
      }
      ...
      try {
        const response = await getDailySalesDetail({
          ...
          marketPlaces: selectedSalesChannels,
    ...
    fetchData();
  }, [isOpen, customDateRange, selectedSalesChannels, selectedProducts]);

  // 데이터가 있는 판매처만 필터링
  const activeChannels = useMemo(() => {
    if (!columnInfo || Object.keys(columnInfo).length === 0) return [];
    return selectedSalesChannels.filter(ch => columnInfo[ch] && columnInfo[ch].length > 0);
  }, [selectedSalesChannels, columnInfo]);
...
export default DailySalesDetailModal;
