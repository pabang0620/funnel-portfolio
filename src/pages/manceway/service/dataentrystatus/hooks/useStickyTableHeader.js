...
export function useStickyTableHeader() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // data-table과 entry-status-table 모두 찾기
      const tables = document.querySelectorAll('.data-table, .entry-status-table');
      ...
      tables.forEach((table, index) => {
        ...
        // 테이블 컨테이너 찾기 (.data-table-container 또는 .entry-status-table-wrapper)
        const tableContainer = table.closest('.data-table-container, .entry-status-table-wrapper');
